...
export const fetchRegions = async (param?: Record<string, string>): Promise<RegionResponse> => {
  ...
  return await fetchClient<RegionResponse>(combineURL(API_URL, "/lpr-regions/get"), {
    method: "GET",
    queryParams: param,
  });
};
    ...
    const filteredData = districts
      .filter((data) =>
        Object.entries(filters).every(([key, value]) => {
          switch (key) {
            case "province_code":
              return data.province_code === value;
            default:
              return true;
          }
        })
        ...
        Object.entries(filters).every(([key, value]) => {
          switch (key) {
            case "province_code":
              return data.province_code === value;
            case "district_code":
              return data.district_code === value;
            default:
              return true;
          }
        })
...
export const fetchPositions = async (param?: Record<string, string>): Promise<OfficerPositions> => {
  ...
  return await fetchClient<OfficerPositions>(combineURL(API_URL, "/person-positions/get"), {
    method: "GET",
    queryParams: param,
  });
};
