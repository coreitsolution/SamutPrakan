...
function Nav() {
  ...
  const fetchCheckpointName = async () => {
    try {
      ...
      if (response.data) {
        setCheckpoint(`${t('text.station')}: ${response.data[0].checkpoint_name || ""}`)
      }
    }
    catch (error) {
      console.error(error)
      ...
      <div className="flex justify-between items-center bg-black">
        ...
        <div className="flex items-center space-x-5 mr-[50px] text-white">
          <div className="flex items-center justify-center w-[150px]">
            <p className="text-center text-[20px] overflow-hidden whitespace-nowrap text-ellipsis w-full" title={authData.userInfo?.username || "User"}>{authData.userInfo?.username || "User"}</p>
          </div>
