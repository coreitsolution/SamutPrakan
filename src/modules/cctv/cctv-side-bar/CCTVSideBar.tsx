...
import { DirectionDetail } from "../../../features/types";
...
const CCTVSideBar: React.FC<CCTVSideBarProp> = ({setCollapse, cameraSetting, setUpdateLastRecognition, setUpdateSpecialPlate}) => {
  ...
  const [LPRCameraSetting, setLPRCameraSetting] = useState<string>("")
  ...
  const [LPRCameraDropdown, setLPRCameraDropdown] = useState<{ label: string, value: string }[]>([])
  ...
  useEffect(() => {
    if (cameraSetting) {
      const dropdownData = cameraSetting.map(({ uid, camera_name }) => ({
        label: camera_name,
        value: uid,
      }))
      const newDropdownData = [ {label: t('text.all'), value: "all"} , ...dropdownData]
      setLPRCameraDropdown(newDropdownData)

      if (LPRCameraSetting === "all" || !newDropdownData.find(item => item.value === LPRCameraSetting)) {
        setLPRCameraSetting("all")
      }
    }
    else {
      const defaultDropdown = [{ label: t('text.all'), value: "" }]
  ...
  useEffect(() => {
    if (lastRecognitionData) {
      ...
      if (lastRecognitionData.is_special_plate === 1) {
  ...
  useEffect(() => {
    if (LPRCameraSetting === "all") {
      setLastRecognitionListData(realtimeData)
    }
    else {
      const filterData = realtimeData.filter((item) => item.camera_uid === LPRCameraSetting)
                  ...
                  >
                    <div className='w-full h-full text-center'>
                      <label className='ml-1'>
                        {`
                          ${isNumber(item.plate_prefix) && isNumber(item.plate_number) ? 
                          `${item.plate_prefix}-${item.plate_number}` : 
                          `${item.plate_prefix} ${item.plate_number}`} 
                          ${i18n.language === "th" ? item.region_name_th : item.region_name_en}
                        `}
                      </label>
                      <div className='flex h-[100px] w-full'>
                        <div className="flex-1 h-full flex items-center justify-center overflow-hidden">
                          <Image
                            key={`vehicle_img_${index}_${item.vehicle_image_url}`}
                            imageSrc={`${IMAGE_URL}${item.vehicle_image_url}`} 
                        ...
                        <div className="flex-1 h-full flex items-center justify-center overflow-hidden">
                          <Image
                            key={`plate_img_${index}_${item.plate_image_url}`}
                            imageSrc={`${IMAGE_URL}${item.plate_image_url}`} 
                    ...
                    >
                      <div className='bg-celti text-center'>
                        <label className="px-1">{dayjs(item.epoch_end).format(i18n.language === 'th' ? 'DD/MM/BBBB HH:mm:ss' : 'DD-MM-YYYY HH:mm:ss')}</label>
                      ...
                      <div className="h-[100px] relative flex flex-col p-1 pl-2">
                        <div className="flex mb-[2px]">
                          ...
                          <span className={`${i18n.language === "en" ? "w-[120px]": "w-[135px]"} truncate`} title={reformatString(item.vehicle_body_type_en)}>{reformatString(item.vehicle_body_type_en)}</span>
                        </div>
                        <div className="flex mb-[2px]">
                          <span className={`${i18n.language === "en" ? "w-[70px]": "w-[55px]"} text-left`}>{t('text.car-brand')}</span>
                          <span className="mx-1">:</span>
                          <span className={`${i18n.language === "en" ? "w-[120px]": "w-[135px]"} truncate`} title={reformatString(item.vehicle_make)}>{reformatString(item.vehicle_make)}</span>
                        </div>
                        <div className="flex mb-[2px]">
                          <span className={`${i18n.language === "en" ? "w-[70px]": "w-[55px]"} text-left`}>{t('text.car-color')}</span>
                          <span className="mx-1">:</span>
                          <span className={`${i18n.language === "en" ? "w-[120px]": "w-[135px]"} truncate`} title={reformatString(item.vehicle_color)}>{reformatString(item.vehicle_color)}</span>
                        </div>
                        <div className="flex mb-[2px]">
                          <span className={`${i18n.language === "en" ? "w-[70px]": "w-[55px]"} text-left`}>{t('text.car-model')}</span>
                          <span className="mx-1">:</span>
                          <span className={`${i18n.language === "en" ? "w-[120px]": "w-[135px]"} truncate`} title={reformatString(item.vehicle_model)}>{reformatString(item.vehicle_model)}</span>
                        </div>
                  ...
                  <tbody className='text-[12px]'>
                    {vehicleCountListData && vehicleCountListData.length > 0
                      ? vehicleCountListData.map((item, index) => (
                          ...
                          >
                            <td className="bg-celtic">{dayjs(item.startHour).format(i18n.language === 'th' ? 'DD/MM/BBBB (HH:mm)' : 'DD/MM/YYYY (HH:mm)')}</td>
                            <td className="bg-tuna">{dayjs(item.endHour).format(i18n.language === 'th' ? 'DD/MM/BBBB (HH:mm)' : 'DD/MM/YYYY (HH:mm)')}</td>
                            <td className="bg-celtic">{item.normalPlateCount}</td>
                            <td className="bg-tuna">{item.specialPlateCount}</td>
                            <td className="bg-celtic">{item.totalCount}</td>
                          </tr>
                        ))
                      : null}
                  </tbody>
                  ...
                  <tbody>
                    {systemStatusListData && systemStatusListData.length > 0
                      ? systemStatusListData.map((item, index) => (
                          ...
                          >
                            <td className='text-start pl-[5px]'>{format(new Date(item.created_at), "HH:mm:ss")} <span>&#62;</span> {item.details}</td>
                          </tr>
                        ))
                      : null}
                  </tbody>
