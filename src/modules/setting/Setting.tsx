  ...
  const fetchCheckpointSetting = async () => {
    try {
      ...
      if (response.success && response.data.length > 0) {
        const provinceName = provinces && provinces.data && provinces.data.find(p => p.province_code === response.data[0].province_code)?.name_th || "";
  ...
  const fetchCameraData = async () => {
    try {
      const response = await fetchClient<CameraSettings>(combineURL(API_URL, "/cameras/get"), {
        method: "GET",
        queryParams: {
          filter: "request_delete=false,deleted=false"
        }
      });
  ...
  const handleScreenSelect = async (event: SelectChangeEvent<number>) => {
    ...
    try {
      if (checkpointData) {
        const updateData = { 
          ...checkpointData,
          live_view_count: value.toString(),
        };
        await fetchClient<CheckpointResponse>(combineURL(API_URL, "/checkpoints/update"), {
          method: "PATCH",
      ...
      <div className='mt-2'>
        <div className='flex justify-between'>
          ...
          {/* <div className='flex gap-2'>
            ...
            <div 
              className={`${checkpointData ? "bg-dodgerBlue cursor-pointer" : "bg-nobel cursor-not-allowed"} rounded-[5px]`}
              title={t('title.add-camera')}
            >
              <Button
                onClick={() => handleCameraButtonClick(true)}
                disabled={!checkpointData ? true : false}
                sx={{
                  textTransform: "capitalize"
                }}
              >
                  ...
                  <input 
                    ...
                    onChange={() => {
                      if (rowSelected.length === cameraDetailSettingData.length) {
                        ...
                        setRowSelected(cameraDetailSettingData.map(item => item.uid))
                      }
                    }}
                  />
                </th>
            ...
            <tbody>
              {cameraDetailSettingData.map((camera, index) => (
                <tr key={camera.id} className="border-b h-[30px] text-[15px] text-white">
                  <td className="px-4 py-2 text-center bg-celtic">
                    <div className="flex items-center justify-center">
                      <input 
                        type="checkbox" 
                        className="w-5 h-5 cursor-pointer"
                        checked={rowSelected.includes(camera.uid)}
                        onChange={() => {
                          if (rowSelected.includes(camera.uid)) {
                            setRowSelected(rowSelected.filter(item => item !== camera.uid))
                          } 
                          else {
                            setRowSelected([...rowSelected, camera.uid])
                          }
                        }}
                      />
                    </div>
                  ...
                  <td className="px-4 py-2 bg-tuna">{camera.camera_name}</td>
                  <td className="px-4 py-2 bg-celtic">{camera.latitude + ", " + camera.longitude}</td>
                  <td className="px-4 py-2 text-end bg-tuna">{formatNumber(camera.detection_count)}</td>
