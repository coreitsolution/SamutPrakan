...
const LocationSetting: React.FC<LocationSettingProps> = ({open, closeDialog, confirmPoint, location}) => {
  ...
  const handleKeyPress = useCallback(
    (event: React.KeyboardEvent<HTMLInputElement>) => {
      if (event.key === "Enter") {
        searchPlaceWithCurrentLocation(searchText)
      }
    },
    [searchText, searchPlaceWithCurrentLocation]
  )
  ...
  const showPosition = (pos: GeolocationPosition) => {
    ...
    searchPlaceWithCurrentLocation(latLng, true);
  };
  ...
  useEffect(() => {
    if (location && location.latitude && location.longitude && searchResults.length === 0 && !isSearching) {
      ...
      searchPlaceWithCurrentLocation(latLng)
    }
  ...
  }, [location, searchPlaceWithCurrentLocation, searchResults, isSearching])
