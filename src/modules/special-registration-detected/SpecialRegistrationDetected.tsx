...
// Constant
import { SEARCH_SPECIAL_ROW_PER_PAGES } from "../../constants/dropdown"
...
const SpecialRegistrationDetected = () => {
  ...
  const [rowsPerPage, setRowsPerPage] = useState(SEARCH_SPECIAL_ROW_PER_PAGES[0])
  const [specialPlateSearchDataList, setSpecialPlateSearchDataList] = useState<LastRecognitionData[]>([])
  const [rowsPerPageOptions] = useState(SEARCH_SPECIAL_ROW_PER_PAGES)
  ...
  const csvData = specialPlateSearchDataList?.length ? [
    ...
    ...specialPlateSearchDataList.map((data) => ({
      ...
      check_point: data.camera_info ? data.camera_info.camera_name : "",
                  ...
                  <table className="w-full text-[15px]">
                    <thead className="sticky top-0 z-10 bg-swamp backdrop-blur-md bg-opacity-80">
                      <tr className="h-[50px] w-full bg-swamp border-none">
                        ...
                        <th className="text-center text-white w-[16%]">{t('table.column.car-type')}</th>
                        <th className="text-center text-white w-[13%]">{t('table.column.model')}</th>
                        ...
                        specialPlateSearchDataList.map((data, index) => {
                          ...
                          return (
                            ...
                            `}>
                              ...
                              <td 
                                className={`flex gap-2 justify-center items-center text-center text-white ${data.is_special_plate ? bgColor : "bg-celtic"}`}
                                ref={el => tdRefs.current[index] = el}
                                onClick={(e) => handleImageClick(e, `${data.plate} ${data.region_info ? data.region_info.name_th : ""}`, data.vehicle_image_url, data.plate_image_url)}
                              >
                                <Image
                                  imageSrc={`${IMAGE_URL}${data.vehicle_image_url}`}
                                  imageAlt={`Vehicle ${index + 1}`}
                                  className="inline-flex items-center justify-center align-middle h-[70px] w-[60px]"
                                />
                                <Image
                                  imageSrc={`${IMAGE_URL}${data.plate_image_url}`}
                                  imageAlt={`Plate ${index + 1}`}
                                  className="inline-flex items-center justify-center align-middle h-[70px] w-[60px]"
                                />
                              </td>
                              <td className={`pl-5 text-start text-white ${data.is_special_plate ? bgColor : "bg-tuna"}`}>{data.camera_info ? data.camera_info.camera_name : "-"}</td>
