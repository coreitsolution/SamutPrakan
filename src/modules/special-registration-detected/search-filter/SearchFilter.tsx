...
// Types
import { FilterSpecialPlates } from "../../../features/types"
...
const SearchFilter: React.FC<SearchFilterProps> = ({setFilterData}) => {
  ...
  const { plateTypes, vehicleColors, vehicleMakes, vehicleModels, regions, vehicleBodyTypes } = useSelector(
  ...
  const filterData: FilterSpecialPlates = {
    ...
    cameraUidList: selectedCheckpoint.map((item) => item.value),
    plateClassId: selectedRegistrationType || 0,
  }
  ...
  useEffect(() => {
    if (plateTypes && plateTypes.data) {
      const options = plateTypes.data.map((row) => ({
        label: row.title_en,
        value: row.id,
      }))
  ...
  }, [plateTypes, i18n.language, i18n.isInitialized])
