...
const ManageSpecialSuspectPerson: React.FC<ManageExtraRegistrationProps> = ({
  ...
  const { provinces, personTypes, prefix, districts, subDistricts } = useSelector(
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (formData.province_code) {
        const res = districts?.data?.filter((district) => district.province_code === formData.province_code)
        if (res) {
          setDistrictsList(res)
        }
      }
      if (formData.district_code) {
        const res = subDistricts?.data?.filter((district) => district.district_code === formData.district_code && district.province_code === formData.province_code)
        if (res) {
    ...
    fetchData();
  }, [formData.province_code, formData.district_code]);
  ...
  const handleSaveClick = async () => {
    ...
    try {
      const updatedFormData: NewSuspectPeople = {
        ...
        province_code: formData.province_code ? formData.province_code : "",
        district_code: formData.district_code ? formData.district_code : "",
        subdistrict_code: formData.subdistrict_code ? formData.subdistrict_code : "",
        ...
        active: formData.active === 1 ? true : false,
  ...
  const handleProvicesChange = (
    ...
    event.preventDefault()
    if (value) {
      handleSelectChange("province_code", value.value);
    }
    else {
      handleSelectChange("province_code", '');
      handleSelectChange("district_code", '');
      handleSelectChange("subdistrict_code", '');
    }
  };
  ...
  const handleDistrictChange = (
    ...
    event.preventDefault()
    if (value) {
      handleSelectChange("district_code", value.value);
    }
    else {
      handleSelectChange("district_code", '');
      handleSelectChange("subdistrict_code", '');
  ...
  const handleSubDistrictChange = (
    ...
    event.preventDefault()
    if (value) {
      handleSelectChange("subdistrict_code", value.value);
