...
const Confirmation: React.FC<ConfirmationProps> = ({setFinalDataList, filesDataList, imagesDataList, textsDataList}) => {
  ...
  const { provinces, dataStatus, personTypes, prefix, districts, subDistricts } = useSelector(
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const results = await Promise.all(textsDataList.map(async (data) => { 
          ...
          const personTitle = prefix?.data?.find((title) => title.title_th.toLowerCase() === data.name_prefix.toLowerCase())

          let district, sub_district
          if (province?.province_code) {
            district = districts?.data?.find((district) => district.name_th === data.district && district.province_code === province.province_code)
          }
          if (district?.district_code) {
            sub_district = subDistricts?.data?.find((subDistrict) => subDistrict.name_th === data.sub_district && subDistrict.province_code === province?.province_code && subDistrict.district_code === district.district_code)
          }
          ...
          return {
            ...
            subdistrict_code: sub_district?.subdistrict_code || "",
    ...
    fetchData();
  }, [textsDataList, provinces, personTypes, prefix, dataStatus, imagesDataList, filesDataList]);
