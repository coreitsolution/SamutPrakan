  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const requiredFields = [
        "name_prefix", "firstname", "lastname", "nation_number",
        "address", "province", "district", "sub_district", "postal_code", "person_class",
      ...
      const validatedData = jsonData.map((row, index) => {
        const missingFields = requiredFields.filter((field) => !row[field as keyof ImportSuspectPeople]);

        // Additional validations for blacklist plate_class
        if (row.person_class?.toLowerCase() === "blacklist") {
      ...
      if (fileImportError) {
        ...
        setTextsData(validatedData as ImportSuspectPeople[]);
      }
      setIsLoading(false)
    }
    catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error)
      PopupMessage("โหลดข้อมูลไม่สำเร็จ", errorMessage, "error");
    }
