...
// Types
import { FilterSpecialSuspectPeople } from "../../../features/types";
...
const SearchFilter: React.FC<SearchFilterProps> = ({setFilterData}) => {
  ...
  const { plateTypes, prefix } = useSelector(
  ...
  const filterData: FilterSpecialSuspectPeople = {
    ...
    selectedRegistrationType: plateTypes?.data?.find((row) => row.id === selectedPersonType)?.title_en || 'all',
  }
